...
class Card {
    constructor(cardId,deckId,cost,damage,health, name, gangability, ability, gang,type, active) {
        this.cardId = cardId;
        this.deckId = deckId;
        this.cost = cost;
        this.damage = damage;
    ...
    static async playDeckCard(game,deckId) {
        try {
            ...
            if (playerShip.ap < card.cost) {
                return {status:400, result:{msg:"Not enough chip points"}};
            }
            ...
            if ((card.cardId == 1 || card.cardId == 2) && playerShip.state.name != "Ready") {
