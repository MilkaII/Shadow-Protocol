...
class Card {
    constructor(crd_id, crd_cost, crd_damage, crd_health, crd_name, crd_gang, crd_type_id) {
        this.crd_id = crd_id;
...
class Deck {
    ...
    static async getDecks() {
        try {
            let [dbDecks] = await pool.query('select * from deck, card, card_type where deck_crd_id = crd_id and crd_type_id = ct_id');
            let decks = [];
